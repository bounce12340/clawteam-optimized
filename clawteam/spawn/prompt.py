...
def build_agent_prompt(
    ...
    lines.extend([
        ...
        f"1. List your tasks: `clawteam task list {team_name} --owner {agent_name}`",
        f"2. Claim each pending task: `clawteam task update {team_name} <task-id> --status in_progress`",
        "3. Execute the task work",
        f"4. Mark done: `clawteam task update {team_name} <task-id> --status completed`",
        "",
