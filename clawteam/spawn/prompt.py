"""Agent prompt builder — identity + task only.

Coordination knowledge (how to use clawteam CLI) is provided
by the ClawTeam Skill, not duplicated here.
"""

from __future__ import annotations


def build_agent_prompt(
    agent_name: str,
    agent_id: str,
    agent_type: str,
    team_name: str,
    leader_name: str,
    task: str,
    user: str = "",
    workspace_dir: str = "",
    workspace_branch: str = "",
    memory_scope: str = "",
) -> str:
    """Build agent prompt: identity + task + optional workspace info."""
    lines = [
        "## Identity\n",
        f"- Name: {agent_name}",
        f"- ID: {agent_id}",
    ]
    if user:
        lines.append(f"- User: {user}")
    lines.extend([
        f"- Type: {agent_type}",
        f"- Team: {team_name}",
        f"- Leader: {leader_name}",
    ])
    if workspace_dir:
        lines.extend([
            "",
            "## Workspace",
            f"- Working directory: {workspace_dir}",
            f"- Branch: {workspace_branch}",
            "- This is an isolated git worktree. Your changes do not affect the main branch.",
        ])
    if memory_scope:
        lines.extend([
            "",
            "## Shared Memory",
            f"- Your team shares memory scope `{memory_scope}`.",
            f"- Use `memory_store` with scope `{memory_scope}` for team-shared knowledge.",
            f"- Use `memory_recall` to access memories stored by other team members in this scope.",
        ])
    lines.extend([
        "",
        "## Task\n",
        task,
        "",
        "## Auto-Start\n",
        "**Start working immediately. Do not wait for inbox messages before beginning your tasks.**",
        "",
        "Your startup sequence:",
        f"1. Send initial heartbeat: `python3 -c \"from clawteam.heartbeat import send_heartbeat; send_heartbeat('{team_name}', '{agent_name}', 'idle')\"`",
        f"2. List your tasks: `clawteam task list {team_name} --owner {agent_name}`",
        f"3. Claim each pending task: `clawteam task update {team_name} <task-id> --status in_progress`",
        "4. Execute the task work",
        f"5. Mark done: `clawteam task update {team_name} <task-id> --status completed`",
        "",
        "## Heartbeat Protocol\n",
        "**You MUST send a heartbeat every 30 seconds to show you are alive.**",
        "",
        "Send heartbeat:",
        f"```bash",
        f"python3 -c \"from clawteam.heartbeat import send_heartbeat; send_heartbeat('{team_name}', '{agent_name}', 'working', '<task-id>', <progress>)\"",
        f"```",
        "",
        "Status options: 'idle', 'working', 'error', 'completed'",
        "Progress: 0-100 percentage",
        "",
        "## Coordination Protocol\n",
        f"- Use `clawteam task list {team_name} --owner {agent_name}` to see your tasks.",
        f"- Starting a task: `clawteam task update {team_name} <task-id> --status in_progress`",
        f"- Finishing a task: `clawteam task update {team_name} <task-id> --status completed`",
        "- When you finish all tasks, send a summary to the leader:",
        f'  `clawteam inbox send {team_name} {leader_name} "All tasks completed. <brief summary>"`',
        "- If you are blocked or need help, message the leader:",
        f'  `clawteam inbox send {team_name} {leader_name} "Need help: <description>"`',
        f"- After finishing work, report your costs: `clawteam cost report {team_name} --input-tokens <N> --output-tokens <N> --cost-cents <N>`",
        f"- Before finishing, save your session: `clawteam session save {team_name} --session-id <id>`",
        "- Send final heartbeat: " + f"`python3 -c \"from clawteam.heartbeat import send_heartbeat; send_heartbeat('{team_name}', '{agent_name}', 'completed')\"`",
        "",
    ])
    return "\n".join(lines)
